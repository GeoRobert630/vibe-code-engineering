"""Safety gate. Runs before the browser is started. Same policy as phase3-runtime-security (no override):

* production: true                        -> refused
* environment named prod/production/live  -> refused
* host label prod/production/live         -> refused
* local hosts (loopback, private IPs, localhost, *.localhost, *.local, *.test, *.internal)
                                          -> allowed for local/development/staging-type environments
* any other host                          -> only for a non-local environment AND target.authorized: true
                                             AND target.authorized_by set
Extra asset origins (assets.allow_origins) must not look like production hosts either.
"""
...
def _prod_label(host: str) -> bool:
    return bool(set(host.lower().replace("-", ".").split(".")) & FORBIDDEN_HOST_LABELS)
...
def evaluate(cfg: Config) -> SafetyDecision:
    ...
    if _prod_label(cfg.host):
        return SafetyDecision(False, f"host '{cfg.host}' looks like a production host", local)
    for origin in cfg.allow_origins:
        if _prod_label(urlsplit(origin).hostname or ""):
            return SafetyDecision(False, f"asset origin '{origin}' looks like a production host", local)
    ...
    if not (cfg.authorized and cfg.authorized_by):
        return SafetyDecision(False, "remote staging targets require target.authorized: true and target.authorized_by "
                                     "(who approved testing)", False)
