...
| Authentication | 3B | `auth_areas.authentication` | **NOT VERIFIED** in the current version |
| Session | 3B | `auth_areas.session` | **NOT VERIFIED** in the current version |
| Authorization (IDOR/BOLA, tenant isolation) | 3C | `authorization` | **NOT VERIFIED** in the current version (status only) |
...
- Keep the 3A namespaces (`RT-HEADERS-*`, `RT-COOKIE-*`, `RT-CORS-*`, `RT-REDIRECT-*`, `RT-TLS-*`, `RT-ERROR-*`)
  separate from the 3B namespaces (`RT-AUTH-*`, `RT-SESSION-*`; reserved, no check generates them yet), the reserved
  3C namespaces (`RT-AUTHZ-*` authorization, `RT-IDOR-*` IDOR/BOLA, `RT-TENANT-*` tenant isolation; never generated by
  runtime probing in this version - they appear only if imported) and ZAP baseline findings (`RT-ZAP-*`).
...
### 6.8 Phase 3C authorization / IDOR / BOLA / tenant-isolation status (status only)

The runtime report has an **Authorization** area (`authorization` in the JSON, "## Authorization" in the Markdown,
`Authorization:` in the reader), shown separately from Authentication and Session. In the current version:

- Runtime authorization testing is **NOT VERIFIED** (possible statuses: PASS / FAIL / NOT CONFIGURED /
  NOT VERIFIED / INCOMPLETE; this version always reports NOT VERIFIED).
- **No actor credentials are consumed** and **no authenticated authorization requests are performed**.
- **No ID enumeration occurs.**
- Cross-user and cross-tenant runtime isolation is **not verified**.
- These areas remain part of the **Layer 2 source-code review** until runtime checks are separately implemented or
  their results imported. `RT-AUTHZ-*`, `RT-IDOR-*` and `RT-TENANT-*` are reserved for such results; this version
  performs no runtime probing and generates none of them, and a NOT VERIFIED status is a coverage gap, not a finding.
...
### Phase 3B
`RT-AUTH-*`, `RT-SESSION-*` (same fields). In the current version there are none: state "Authentication and
Session NOT VERIFIED - Phase 3B is plumbing only; no credentials read, no login/logout requests sent".
Keep the 3A and 3B namespaces separate.

### Phase 3C
Authorization status (currently "NOT VERIFIED - runtime authorization, IDOR/BOLA and tenant-isolation checks are not
executed; covered only by Layer 2 source-code review"), including the Authorization, IDOR/BOLA and Tenant isolation
sub-areas (all NOT VERIFIED). `RT-AUTHZ-*`, `RT-IDOR-*` and `RT-TENANT-*` findings only if imported from a separate
tool; these namespaces are reserved and none are generated by runtime probing in this version.
...
Always includes Authentication and Session (NOT VERIFIED - Phase 3B plumbing only) until the Phase 3B
authentication runtime checks exist and are independently tested, and Authorization / IDOR / BOLA / tenant isolation
(NOT VERIFIED - Phase 3C status only) until runtime authorization checks are implemented or imported.
